...
const getCredentials = request => {
  ...
  if (autho === undefined || autho === '' || !autho.startsWith("Basic")) {
    return null;
  }
  const encodeHeader = autho.split(" ")[1];
  // Not properly encoded
  if (encodeHeader.includes(':')) {
    return null;
  }
  const buff = Buffer.from(encodeHeader, 'base64');
 ...
 *
 * @param {http.IncomingMessage} request HTTP request message sent by the client to a server
 * @returns {boolean} whether JSON is accepted as a response by the client or not
 */
 ...
 *
 * @param {http.IncomingMessage} request HTTP request message sent by the client to a server
 * @returns {boolean} whether JSON is the content type of the client request or not
 */
...
module.exports = { getCredentials, acceptsJson, isJson, parseBodyJson };