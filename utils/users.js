 ...
 * 
 * @returns {string} a random string used as an unique ID for new registering user
 */
const generateId = () => {
 ...
 *
 * @param {string} email supplied email created by a registering user
 * @returns {boolean} whether email is already used by any other users or not 
 */
 ...
 *
 * @param {string} email supplied email from the logging in user
 * @param {string} password supplied password from the logging in user
 * @returns {object|undefined} copy of the found user in the database with the supplied email and password or undefined if the user is not found
 */
const getUser = (email, password) => {
  const user = data.users.find(viewingUser => viewingUser.email === email && viewingUser.password === password);
  return user && {...user };
};
 ...
 * to prevent modifying the user outside of this module.
 *
 * @param {string} userId supplied ID
 * @returns {object|undefined} copy of the user in the database with that ID or undefined if the user is not found  
 */
const getUserById = userId => {
  // TODO: 8.4 Find user by user id
  const user = data.users.find(viewingUser => viewingUser._id === userId);
  return user && {...user };
};
...
const deleteUserById = userId => {
  ...
  const userIndex = data.users.findIndex(deletingUser => deletingUser._id === userId);
 ...
 *
 * @param {object} user object containing data of new created user
